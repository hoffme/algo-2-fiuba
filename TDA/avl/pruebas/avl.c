...
size_t actualizar_altura(avl_t* arbol, nodo_t* nodo, nodo_t* padre, const char* buscado, bool f_accion (avl_t*, nodo_t*, nodo_t*, const char*, void*), void* extra, bool* estado);
...
bool avl_guardar(avl_t* arbol, const char* clave, void* dato) {
	bool estado = true;
	size_t altura = actualizar_altura(arbol, arbol->raiz, NULL, clave, insertar_dato, dato, &estado);
...
void* avl_borrar(avl_t *arbol, const char* clave) {
	bool estado = true;
	void* borrado = NULL;
	size_t altura = actualizar_altura(arbol, arbol->raiz, NULL, clave, borrar_dato, borrado, &estado);
	if (arbol->raiz) arbol->raiz->altura = altura;
	if (estado) return borrado;
	return NULL;
}
...
size_t actualizar_altura(avl_t* arbol, nodo_t* nodo, nodo_t* padre, const char* buscado, bool f_accion (avl_t*, nodo_t*, nodo_t*, const char*, void*), void* extra, bool* estado) {
	...
	if (dir > 0) nodo->altura = max(actualizar_altura(arbol, nodo->izq, nodo, buscado, f_accion, extra, estado), nodo_altura(nodo->der)) + 1;
	else if (dir < 0) nodo->altura = max(actualizar_altura(arbol, nodo->der, nodo, buscado, f_accion, extra, estado), nodo_altura(nodo->izq)) + 1;
	else {
		*estado = f_accion(arbol, padre, nodo, buscado, extra);
...
bool borrar_dato(avl_t* arbol, nodo_t* padre, nodo_t* nodo, const char* clave, void* dato_borrado) {
	...
	else  borrar_simple_hijos(arbol, padre, nodo);
	--arbol->cant;
