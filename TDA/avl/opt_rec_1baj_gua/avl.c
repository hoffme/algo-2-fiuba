...
typedef bool (*f_accion_t) (avl_t* arbol, nodo_t* padre, nodo_t* nodo, const char* buscado, void* extra);
...
size_t buscar_y_aplicar(avl_t* arbol, nodo_t* nodo, nodo_t* padre, const char* buscado, f_accion_t f_accion, void* extra, bool* estado);

long max(long ent1, long ent2);

long mod(long ent1);

bool insertar_dato(avl_t* arbol, nodo_t* padre, nodo_t* nodo, const char* clave, void* dato);

bool borrar_dato(avl_t* arbol, nodo_t* padre, nodo_t* nodo, const char* clave, void* dato_borrado);
...
bool avl_guardar(avl_t* arbol, const char* clave, void* dato) {
	bool ok = true;
	size_t altura = buscar_y_aplicar(arbol, arbol->raiz, NULL, clave, insertar_dato, dato, &ok);
	if (arbol->raiz) arbol->raiz->altura = altura;
	if (ok) return true;
	return false;
}
...
void* avl_borrar(avl_t *arbol, const char* clave) {
	bool ok = true;
	void* borrado = NULL;
	size_t altura = buscar_y_aplicar(arbol, arbol->raiz, NULL, clave, borrar_dato, borrado, &ok);
	if (arbol->raiz) arbol->raiz->altura = altura;
	if (ok) return borrado;
	return NULL;
}
...
size_t buscar_y_aplicar(avl_t* arbol, nodo_t* nodo, nodo_t* padre, const char* buscado, f_accion_t f_accion, void* extra, bool* estado) {
	if (!nodo) {
		*estado = f_accion(arbol, padre, nodo, buscado, extra);
		return nodo_altura(nodo);
	}

	int dir = arbol->f_comp(nodo->clave, buscado);
	if (dir > 0) nodo->altura = max(buscar_y_aplicar(arbol, nodo->izq, nodo, buscado, f_accion, extra, estado), nodo_altura(nodo->der)) + 1;
	else if (dir < 0) nodo->altura = max(buscar_y_aplicar(arbol, nodo->der, nodo, buscado, f_accion, extra, estado), nodo_altura(nodo->izq)) + 1;
	else {
		*estado = f_accion(arbol, padre, nodo, buscado, extra);
		if (nodo) nodo->altura = nodo_altura(nodo);
		else return 0;
	}
...
bool borrar_dato(avl_t* arbol, nodo_t* padre, nodo_t* nodo, const char* clave, void* dato_borrado) {
	...
	if (nodo->izq && nodo->der) {
		printf("BORRANDO 2\n");
		borrar_dos_hijos(arbol, nodo);
	} else {
		printf("BORRANDO 1 o 0\n");
		borrar_simple_hijos(arbol, padre, nodo);
	}
	--arbol->cant;
...
bool insertar_dato(avl_t* arbol, nodo_t* padre, nodo_t* nodo, const char* clave, void* dato) {
	...
	if (!padre) arbol->raiz = n_nodo;
		...
		int dir = arbol->f_comp(padre->clave, clave);
		if (dir > 0) padre->izq = n_nodo;
