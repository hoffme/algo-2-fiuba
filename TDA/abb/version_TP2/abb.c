...
void _nodo_rango_insertar_en_lista(abb_cmp_rango_t f_comp, nodo_t* nodo, void* desde, void* hasta, lista_t* lista) {
	if (!nodo) return;

	bool izq = f_comp(desde, nodo->clave) <= 0;
	bool der = f_comp(hasta, nodo->clave) >= 0;

	if (izq) _nodo_rango_insertar_en_lista(f_comp, nodo->izq, desde, hasta, lista);
	if (izq && der) lista_insertar_ultimo(lista, nodo->clave);
	if (der) _nodo_rango_insertar_en_lista(f_comp, nodo->der, desde, hasta, lista);
}
...
lista_t* abb_rango_lista(abb_t *arbol, abb_cmp_rango_t f_comp, void* desde, void* hasta) {
	...
	_nodo_rango_insertar_en_lista(f_comp, arbol->raiz, desde, hasta, lista);
