...
bool redimensionar(heap_t* heap, bool agrandar);
...
bool heap_encolar(heap_t *heap, void *elem){
	...
	if (heap->cant + 1 >= heap->tam && !redimensionar(heap, true)){
		return false;
	}
...
void* heap_desencolar(heap_t *heap){
	...
	if (heap->tam > TAM_INI && heap->cant <= heap->tam / MIN_CANT) redimensionar(heap, false);
